...
PLACEMENT (critical): this overlays a TALKING-HEAD video. The SPEAKER occupies the CENTER third of the frame (x from 33% to 67%, full height). Keep the ENTIRE graphic OUT of that center third. Concretely: a card/lockup must fit inside the LEFT third (its right edge at or left of 33% width) OR the RIGHT third (its left edge at or right of 67% width) — a left card that is too wide and spills past 33% into the center is NOT acceptable, so keep cards narrow. The ONLY element allowed to span the center is a slim FULL-WIDTH band in the bottom ~18% of the frame (a true lower-third strip that sits below the speaker's face). NEVER put a graphic dead-center over the speaker. A full-frame style (a grid/layout meant to reframe the shot) is the only exception.
