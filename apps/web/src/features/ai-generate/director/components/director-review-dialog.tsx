...
/**
 * The Director cut-review modal (U4): lists proposed cut/take/repeat ops with
 * accept/reject; applies the accepted ops as one undoable step and seeds the
 * per-category taste. Rendered once at the editor root.
 *
 * Vestigial as of R1: production Director runs go through the docked
 * `DirectorCutPanel` (`openCutPanel`) instead of `openWith`, so this modal never
 * actually opens today; it stays available for a future non-docked caller. The
 * Highlight modal path is fully RETIRED (R1): `openHighlight` now populates the
 * docked `DirectorHighlightPanel` instead of this dialog, so there is no highlight
 * branch here anymore.
 */
