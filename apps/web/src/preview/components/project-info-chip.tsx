...
/**
 * Project info chip: the compact preview-toolbar trigger for project name,
 * frame rate, and aspect ratio (moved out of Settings -> Project info, per
 * the menu IA audit -- this data belongs near the canvas it describes, not
 * buried in a settings tab). CapCut/Premiere convention: an aspect-ratio
 * chip next to the zoom control.
 */
...
export function ProjectInfoChip() {
	...
	const currentRatio = dimensionToAspectRatio(currentCanvasSize);
	const chipLabel = isCustomSelected
		? "Custom"
		: (PRESET_LABELS[currentRatio] ?? currentRatio);
		...
		<Popover>
			...
			<PopoverContent align="end" sideOffset={8} className="w-72 p-0">
				...
				<div className="flex flex-col gap-1 px-2 py-2.5">
					<span className="text-sm font-medium px-1.5 pb-1">Aspect ratio</span>
